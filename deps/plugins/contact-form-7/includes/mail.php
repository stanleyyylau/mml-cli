...
function wpcf7_special_mail_tag( $output, $name, $html ) {
	...
	if ( '_remote_ip' == $name ) {
		if ( $remote_ip = $submission->get_meta( 'remote_ip' ) ) {
			// MML: Insert code here to get location of the IP
			// We will use this API http://ip-api.com/json/208.80.152.201
			$requestUrl = 'http://ip-api.com/json/' . $remote_ip;
			$response = wp_remote_get( esc_url_raw($requestUrl) );
			if ( is_array( $response ) ) {
			  $header = $response['headers'];  
			  $responseBody = json_decode( wp_remote_retrieve_body( $response ), true ); 
			  // If the API status is success
			  if ($responseBody['status'] == 'success') {
				  $responseInString = 'IP Address: ' . $responseBody['query'] . '<br>City: ' . $responseBody['city'] . '<br>Region: ' . $responseBody['regionName'] . '<br>Country: ' . $responseBody['country'] ;
				  return $responseInString; 
			  } else {
				  // Or we just return original IP (No Location Data Found)
				  return $remote_ip . ' (No Location Data Found)';
			  }
			  
			}
			// If we don't get a response from the location API, return just the ip address
			return $remote_ip . ' (Fail to get location Data)';
		} else {
			return '';
		}
	}
